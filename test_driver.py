...
def ode(w, t):
	return [w[2], w[3], Sc1.Fx(t) / Sc1.s.mass(), Sc1.Fy(t) / Sc1.s.mass()]
...
def test_get_shape_circleScene():
	...
	assert Sc1.get_shape().cm_x() == expected[0] and Sc1.get_shape().cm_y() == expected[1] and Sc1.get_shape().mass() == expected[3]
...
def test_sim_time():
	experimentalt, experimentaln = Sc1.sim(10, 100)
	assert [(i * 10 / (100 - 1)) for i in range(0, 100 - 1)]  == expectedt

def test_sim_wsol1():
	experimentalt, experimentaln = Sc1.sim(10, 100)
	actual = odeint(ode, [c.cm_x(), c.cm_y(), 0, 0],[(i * 10 / (100 - 1)) for i in range(0, 100 - 1)])
	arr1 = np.array(experimentaln)
	arr2 = np.array(actual)
	out_arr = np.subtract(arr1, arr2)
	assert LNG.norm(out_arr) < 0.01
